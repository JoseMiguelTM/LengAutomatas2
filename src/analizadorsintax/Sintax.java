...
public class Sintax {
    ...
    int posicion = 0;
    Arbol sauceLloron;
    Stack<NodosPila> pila = new Stack<NodosPila>();
    Stack<NodosPila> pilaCuerpo = new Stack<NodosPila>();
    Stack<NodosPila> pilaDecla = new Stack<NodosPila>();
    ...
    public void buscarReglas() {
        ...
        switch (primerElm) {
            case "principal":
                ...
                NodosPila n1 = new NodosPila("raiz", nodoPrograma);
                pila.push(n1); //inserto el nodo raiz a la pila
                /*
    ...
    public boolean ReglaPrograma(String token) {
        ...
        if (token.equals("principal")) {
            ...
            NodosPila n1 = new NodosPila("der", new Nodo("Principal", true)); //Nodo "derecho" Principal
            pila.push(n1); //Agrego a la pila
            ...
            if (elementos.get(posicion).equals("[")) {
                ...
                NodosPila n2 = new NodosPila("der", new Nodo("CA", true)); //Nodo "derecho" corchete apertura
                pila.push(n2);
                
                posicion++;
                if (ReglaCuerpo(elementos.get(posicion), "ReglaPrograma")) {
                    NodosPila nCuerpo = new NodosPila("der", new Nodo("Cuerpo", false));
                    pila.push(nCuerpo);
                    if (elementos.get(elementos.size() - 1).equals("]")) {
                        ...
                        NodosPila nc = new NodosPila("hoja", new Nodo("CC", true));
                        pila.push(nc);
    ...
    public boolean ReglaCuerpo(String token, String sel) {
        pilaCuerpo.push(new NodosPila(pila.firstElement().getInd(), new Nodo("Cuerpo", false))); //Se añade la raiz de la otra pila Cuerpo
        /* ANTERIOR BLOQUE
        ...
        switch (token) {
            ...
            default:
                ...
                pilaCuerpo.push(new NodosPila("UnicoNodo", new Nodo("Decla", false)));
                break;
        }
    ...
    public boolean ReglaVariable(String token) {
        ...
        if (Pattern.matches(regex, token)) {
            ...
            nodoDecla.hijos.add(nodoVar); //Añado el "nodoVar" al "nodoDecla" porque es su hijo
